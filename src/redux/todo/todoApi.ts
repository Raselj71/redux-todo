...
export const todosApi = apiSlice.injectEndpoints({
  ...
  endpoints: (builder) => ({
    getTodos: builder.query<ListResponse, { page?: number, limit?: number, status?: string, search?: string, sortBy?: string, sortOrder?: 'asc'|'desc' }>({
    ...
    createTodo: builder.mutation<Todo, todoResponse>({
      query: (body) => ({ url: 'todos', method: 'POST', body }),
      invalidatesTags: [{ type: 'Todos', id: 'LIST' }]
    }),
    updateTodo: builder.mutation<Todo, { id: string, patch: Partial<Todo> }>({
      ...
      async onQueryStarted({ id, patch }, { dispatch, queryFulfilled }) {
        ...
        try { await queryFulfilled } catch { patchResults.undo() }
      },
